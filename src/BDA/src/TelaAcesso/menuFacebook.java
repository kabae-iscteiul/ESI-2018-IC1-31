...
public class menuFacebook extends JFrame {
	 ...
	 * @throws Exception
	 */
	public menuFacebook(String name, String accessToken) {
		this.name = name;
	...
	/**
	 * Imprimir as publicações do utilizador
	 * @throws Exception 
	 */
	public void printPosts() throws Exception {
		descarregarPosts();
	...
	/**
	 * Descarrega os posts onlime mode
	 * @throws Exception 
	 */
	public void descarregarPosts() {
		...
		try {
			...
			result = (new DefaultFacebookClient(accessToken, Version.LATEST)).fetchConnection("me/feed", Post.class);
			xm = new XmlMenu();
			for (List<Post> page : result) {
				for (Post aPost : page) {
					if (aPost.getMessage() != null) {
						listaPublicacoes.add(aPost);
						xm.addPost(name, aPost.getMessage(), "FacebookPost");
					}	
				}
			}
		} catch (Exception e) {
	...
	/**
	 * Imprime na interface grafica os posts gravados no ficheiro XML
	 * @throws Exception
	 */
	public void printSavedPosts() throws Exception {
		DefaultListModel<String> posts = new DefaultListModel<String>();
	
		XmlMenu xm = new XmlMenu();
		
		Node no = xm.getDoc().getElementsByTagName("FacebookPost").item(0);
		NamedNodeMap nnm = no.getAttributes();  
		for(int i=0; i<nnm.getLength(); i++) {
			posts.addElement(nnm.item(i).getTextContent());
		}
	 ...
	 * @param mails
	 */
	public void trocarDeInformacao(DefaultListModel<String> posts) {
		System.out.println(posts.getElementAt(0));
		list.setModel(posts);
	 ...
	 * @param assunto
	 */
	public void tratarJanelaMensagem(String assunto) {
	...
	static class EnviarPostGUI extends JFrame {
		...
		private static final long serialVersionUID = 962589476057970063L;
		private String accessToken;
		private JFrame frame;
		private JTextField textField;
		private String grp;

		public EnviarPostGUI(String accessToken2) {
			this.accessToken = accessToken2;
			abrirJanelaEnviar();
		}

		/**
		 * Abre uma janela para publicar um post
		 */
		...
		/**
		 * Publica o post escrito
		 */
		public void publishPost() {

			Connection<Group> result = (new DefaultFacebookClient(accessToken, Version.LATEST)).fetchConnection("me/groups",
					Group.class); 
	...
	static class GUIMostrarPublicacao extends JFrame {
		...
		/**
		 * Abre uma janela com a informação do post escolhido
		 */
